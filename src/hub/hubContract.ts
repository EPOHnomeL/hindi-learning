...
/**
 * The behavioural contract every HubRepository adapter must satisfy. Run it
 * against the in-memory adapter and (when DATABASE_URL_TEST is set) the Neon
 * adapter. `makeHub` must return a FRESH, EMPTY hub on each call.
 */
export function runHubContract(label: string, makeHub: () => Promise<HubRepository>) {
  describe(`HubRepository contract — ${label}`, () => {
