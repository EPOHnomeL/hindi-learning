...
export function SettingsDialog({ onClose }: { onClose: () => void }) {
  ...
  // A Seat has no email and no display name in use (ADR 0031). The name field is
  // real-name shaped, and a real name is the one thing this rail's mitigation rests
  // on NOT storing, so it is absent for a Seat rather than merely unused.
            ...
            {LOCALES.map((code) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-between gap-3 rounded-lg px-3 py-2 text-start transition-colors ${
