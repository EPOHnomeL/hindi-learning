...
// to one line before measuring. A blank mission is no mission — null, so the panel
// renders without an empty gap. Over the limit: cut on the last word boundary and
// mark it with an ellipsis; a single word longer than the limit is hard-cut,
// because there is nothing to break on.
