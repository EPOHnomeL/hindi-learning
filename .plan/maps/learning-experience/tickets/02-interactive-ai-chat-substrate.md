...
Merged from 4 separate scope tickets (formerly the `ai-chat` effort's 01–04, GitHub #54–#57)
— they're one dependency chain
...
## Context folded from the retired `ai-chat` map (2026-08-01)

<!-- was .plan/maps/learning-experience/tickets/02-interactive-ai-chat-substrate.md; that single-ticket map was consolidated into pedagogy -->

- Destination: a locked **serving-path and metering decision** for a persistent, course-wide,
  1-on-1 learner chat — the interaction plane that debate, guided discussion, and
  apply-outcomes coaching all ride on. This produces the substrate decision (likely an ADR)
  and a CONTEXT.md term, not the chat.
- **Domain:** Question, Reply, Response, Routine (CONTEXT.md). The proposed new term is this
...
- The three conversation *formats* were scoped separately and closed on GitHub as
  ...
  is folded into this ticket's body — grill against it, don't re-chart it.
- Seams: [Scope learning-pyramid phases](01-scope-learning-pyramid-phases.md) (discussion is
  a pyramid modality), [Scope experiential learning](02-scope-experiential-learning.md)
  (coaching = conversation, assignment = structure), and
  [Knowledge grilling](05-diagnostic-mode-and-teach-me-that-handoff.md) (also wants an
  always-on responder — likely the same substrate).
- Skills: `/grilling` + `/domain-modeling`, `convex:convex-expert`, `convex:agent` if the
  `@convex-dev/agent` component is on the table.
- **Fog:** grounding mechanics (context-stuffing vs RAG — probably graduates into its own
  ticket once the serving path is fixed); whether the Routine reads chat history for ZPD
  signal — a real change to the teaching loop's inputs, sharper once persistence is decided.
- **Out of scope:** multi-user rooms. Explicitly resolved 2026-07-15: this is 1-on-1.

<!-- Moved 2026-09-01 from `pedagogy/04` during the .plan consolidation (33 map dirs to 7 active maps).
