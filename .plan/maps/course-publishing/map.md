# Course publishing, tenant catalogue & free self-enroll
...
spec carry in-place amendment notes; **issue 11 (the per-tenant `selling` flag) is still unbuilt.**
...
- [Model free self-enroll](tickets/01-model-self-enroll-grant.md) — self-enroll is a **new fifth access
...
- [Define the "publish" action](tickets/03-define-publish-action.md) — **`published` is a course lifecycle
...
- [PRD + implementation issues](tickets/06-prd-and-issue-breakdown.md) — the convergence ticket. Synthesized
  all eight decisions into **[spec.md](spec.md)** and eight dependency-ordered, `/tdd`-sized
...
- **Full app-UI (chrome) translation** ("the entire app translation going forward") — localizing the
  ...
  [ticket 07](tickets/07-language-scoped-access.md) surfaced that the *content-language* half was obsolete
...
- **Learner-progress percentage & lesson-estimate accuracy** (surfaced resolving
  [ticket 03](tickets/03-define-publish-action.md), ruled out 2026-07-18) — the moving-denominator churn
