...
<!-- was .plan/maps/media-generation/tickets/01-scope-course-trailer.md; the course-media map was consolidated into media-generation -->

- **This ticket and [Scope course audio](02-scope-course-audio.md) pull in opposite
  directions** and must not be collapsed: the trailer is *marketing*, outward, per-course,
  owner-triggered; audio is *pedagogy*, inward, per-lesson, learner-consumed.
- **The provider-split constraint decides half of this ticket:** Convex actions cannot run
...
- The course already carries rich structured ingredients for a template — title, Emblem,
  Mission, lesson list, sample quiz, lesson count, languages. The cheapest branch is a
  template-driven motion *page*, which raises the honest question of whether an mp4 is
  needed at all.
- Branding: a tenant's trailer must look like that tenant (whitelabel theming) — this is what
  turns "one build" into "four".
- **Not the same as**
  [Scope the onboarding & marketing video](03-scope-onboarding-and-marketing-video.md):
...
- **Out of scope:** paid-ads or distribution tooling; video renderings of lesson content —
  that's [rich-media](../../rich-media/map.md).
