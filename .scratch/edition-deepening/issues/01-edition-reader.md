# edition-deepening/01: The Edition reader — `lib.loadEdition`

**Status:** done
**Assignee:** wayfinder session (claimed 2026-07-20)
...
**Deletion test:** passes — deleting the helper makes the rule reappear across ~11 files / ~38 sites.

**Wins (codebase-design):** locality (fallback bugs concentrate in one module); leverage (one interface, 5 call
sites); deletes `public.editionMap`; interface stays small, implementation absorbs the 38 sites; one test surface.

Independent of ticket 02 (that's the grant/access read; this is the content projection). Enabling seam for the
fogged content/public collapse.
...
**Then:** TDD `loadEdition`; migrate the call sites (content.ts, public.ts — delete its dup
`editionMap`, capture.myQuestions, shares, certificates); delete `content.trOne`/`content.editionMap`
once unused; existing per-file tests are the regression net, prune the ones made redundant.

---

## Resolution (2026-07-20 — done)
