...
Verified 2026-07-10: the migration path is still live in code (convex/whitelist.ts:147 reads `AUTH_ALLOWED_EMAILS`), and nothing in the repo records whether the prod run + env unset happened. Whether this is done is an ops check on the prod deployment — left open as `ready-for-human`.
