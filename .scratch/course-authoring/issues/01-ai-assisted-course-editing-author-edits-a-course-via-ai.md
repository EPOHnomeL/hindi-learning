# course-authoring/01: AI-assisted course editing (author edits a course via AI)

**Status:** open
**Depends on:** the review/approve gate (new — does not exist today); for the live-chat shape, [ADR 0014](../../../docs/adr/0014-provider-agnostic-teaching-runtime-two-lines.md)
**Imported:** from GitHub #16 on 2026-07-15 (created 2026-07-10; GitHub issue deleted after import)

> Migrated from [`.scratch/course-authoring/issues/01-ai-assisted-course-editing.md`](https://github.com/EPOHnomeL/hindi-learning/blob/93ad1e399b426e882c40d9422d8691e1dfb3a46b/.scratch/course-authoring/issues/01-ai-assisted-course-editing.md) on 2026-07-10. Relative links in the text resolve against that file's location.
...
### Addendum (2026-07-15): feedback → regenerate a lesson

From the feature-notes grilling: "Feedback and regenerate lesson" is **owner-side** (the
user's call — not a learner affordance), so it folds in here as a first-class edit-intent
rather than a separate feature: the owner gives feedback on a specific Lesson ("too hard",
"wrong emphasis", free text) and the AI authors a **superseding** lesson from it (the
`supersededBy` machinery exists; nothing AI-driven uses it yet). Notes for triage:

- "Regenerate" = supersede, never mutate (ADR 0003) — the feedback text is the brief for the
  replacement.
- This is the simplest edit-intent of the set — likely the first one to build, and a good
  probe of the async-vs-live shape decision above.
- Learner-side feedback stays out (resolved 2026-07-15); learners have [[Question]]s.

## Comments

### EPOHnomeL — 2026-07-10

Verified 2026-07-10 (main @ 1b2db94) — still outstanding: no edit-intent entity or mutation exists, and there is no draft/review/approve gate — `publishLesson` inserts immediately-live rows (content.ts:834-870). Supersede mechanics exist but nothing AI-driven uses them. The review gate is the shared dependency with the studio reader-visibility issue (#23).
