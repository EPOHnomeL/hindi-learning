# course-authoring/02: Direct (manual) course editing

**Status:** needs-triage (to-scope — captured 2026-07-08; not built)
**Depends on:** the review/approve gate (new — Lessons auto-publish today), shared with [issue 01](01-ai-assisted-course-editing-author-edits-a-course-via-ai.md)
**Labels:** needs-triage
**Imported:** from GitHub #17 on 2026-07-15 (created 2026-07-10; GitHub issue deleted after import)

> Migrated from [`.scratch/course-authoring/issues/02-direct-course-editing.md`](https://github.com/EPOHnomeL/hindi-learning/blob/93ad1e399b426e882c40d9422d8691e1dfb3a46b/.scratch/course-authoring/issues/02-direct-course-editing.md) on 2026-07-10. Relative links in the text resolve against that file's location.

## Why

Vocabulary: [`CONTEXT.md`](../../../CONTEXT.md) (Topic, Lesson, Reference, Mission, Resource, Frontier). Relates to [ADR 0003](../../../docs/adr/0003-immutable-lessons-mutable-references.md) (immutable Lessons, mutable References). The **AI-assisted** counterpart is [issue 01](01-ai-assisted-course-editing-author-edits-a-course-via-ai.md) — this is the hands-on, direct-manipulation path.

Let a course owner **edit their course directly** in the app — by hand, not by instructing the AI (that's [issue 01](01-ai-assisted-course-editing-author-edits-a-course-via-ai.md)). Extend the editing that already exists into a real course-editing surface.
...
- All edits are **owner-only**; a **review/approve gate** applies before shared / company-wide readers see changes (shared with [issue 01](01-ai-assisted-course-editing-author-edits-a-course-via-ai.md) — the gate does not exist today; Lessons auto-publish).
- An owner edit never silently changes content a reader has already completed without a visible version bump.

## Notes

- Distinct from [issue 01](01-ai-assisted-course-editing-author-edits-a-course-via-ai.md) (AI-assisted "direct and build"). This ticket is the manual editor: buttons and forms, no agent.
...
**Verified 2026-07-10 (main @ 1b2db94) — partially stale since the 2026-07-08 audit.**

Already shipped: in-place prose editing via hover pencil for source lessons (content.ts:234-343), references (content.ts:426-449), and translated Editions (content.ts:457-550); lesson delete with full cascade (`deleteLesson`, content.ts:357, commit bce05ab + 7e9ddc1); rename/Mission editing; resource add.

**Actual remaining scope:** reorder lessons (no resequence mutation; `seq` fixed at publish), edit quiz questions (`editLesson` refuses quiz-structure changes, content.ts:322-332), remove a resource (no mutation exists), and the review/approve gate (shared with #16).
