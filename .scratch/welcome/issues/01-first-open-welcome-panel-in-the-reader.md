# welcome/01: First-open welcome panel in the reader (signed-in + Public link)

**Status:** open
**Depends on:** — (related: #46 "Improve Onboarding Flow", #85 progress-feature)
...
- A dashboard-wide welcome or product tour — #46 owns the broader onboarding flow.
- Real per-lesson progress semantics beyond the existing `opened`/`completed`
  rows — #85 owns the Progress feature. This issue consumes what exists and must
...
- Open questions for triage:
  ...
  - Should a signed-in learner opening their *very first lesson ever* (any course)
    ...
    per-course only; the account-level story belongs to #46.
